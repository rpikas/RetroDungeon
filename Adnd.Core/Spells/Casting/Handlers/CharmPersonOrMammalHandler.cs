...
public sealed class CharmPersonOrMammalHandler : ISpellEffectHandler
{
    public bool CanHandle(string spellId)
        => string.Equals(spellId, "charm_person_or_mammal_druid", StringComparison.OrdinalIgnoreCase)
           || string.Equals(spellId, "charm_person_or_mammal", StringComparison.OrdinalIgnoreCase);
    ...
    public SpellCastResult Resolve(SpellCastRequest request)
    {
        ...
        if (request.Context != SpellUseContext.Combat)
            return SpellCastResult.Failure("Charm Person or Mammal can only be cast in combat.");
        ...
        if (!IsHumanoidOrAnimal(target.Template.Type))
        {
            result.Events.Add($"{target.DisplayName} is not a humanoid or animal. {spell.Name} has no effect.");
            return result;
        }
        ...
        result.Events.Add($"{target.DisplayName} now regards {request.Caster.Name} as a trusted friend and will aid the druid's side in combat.");
        return result;
    }

    private static bool IsHumanoidOrAnimal(MonsterType type)
        => type is MonsterType.Humanoid or MonsterType.Animal;
