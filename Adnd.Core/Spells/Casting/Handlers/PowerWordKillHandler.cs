...
public sealed class PowerWordKillHandler : ISpellEffectHandler
{
    public bool CanHandle(string spellId) => string.Equals(spellId, "power_word_kill", StringComparison.OrdinalIgnoreCase);
    ...
    public SpellCastResult Resolve(SpellCastRequest request)
    {
        ...
        if (request.Context != SpellUseContext.Combat)
            return SpellCastResult.Failure("Power Word Kill can only be cast in combat.");
        ...
        result.Events.Add($"{target.DisplayName} is slain instantly (no save). HP {before}->0.");
