...
public sealed class PowerWordStunHandler : ISpellEffectHandler
{
    public bool CanHandle(string spellId) => string.Equals(spellId, "power_word_stun", StringComparison.OrdinalIgnoreCase);
    ...
    public SpellCastResult Resolve(SpellCastRequest request)
    {
        ...
        if (request.Context != SpellUseContext.Combat)
            return SpellCastResult.Failure("Power Word: Stun can only be cast in combat.");
        ...
        if (target.CurrentHitPoints >= 51)
        {
            result.Events.Add($"{target.DisplayName} has {target.CurrentHitPoints} HP and is unaffected.");
            return result;
        }

        var rounds = rng.Next(1, 5);
        target.SetStatus(MonsterStatus.Paralyzed, rounds);
        result.Events.Add($"{target.DisplayName} is stunned and incapacitated for {rounds} round(s) (no save).");
