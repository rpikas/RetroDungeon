...
public sealed class ProtectionFromFireHandler : ISpellEffectHandler
{
    public bool CanHandle(string spellId)
        => string.Equals(spellId, "protection_from_fire", StringComparison.OrdinalIgnoreCase);
    ...
    public SpellCastResult Resolve(SpellCastRequest request)
    {
        ...
        if (string.Equals(target.Name, request.Caster.Name, StringComparison.OrdinalIgnoreCase))
        {
            ...
            target.SetProtectionFromFireSelf(rounds, absorption);
            ...
            return new SpellCastResult
            {
                ...
                Events =
                {
                    ...
                    $"{target.Name} gains immunity to normal fire and magical fire absorption pool {absorption} HP for {rounds} rounds."
                }
            };
        }

        target.SetProtectionFromFireOther(rounds);
        ...
        return new SpellCastResult
        {
            ...
            {
                $"{request.Caster.Name} casts {spell.Name}. {spell.EffectDescription}",
                $"{target.Name} gains normal-fire immunity, +4 saves vs fire, and 50% damage from magical fire for {rounds} rounds."
            }
        };
    }
}
