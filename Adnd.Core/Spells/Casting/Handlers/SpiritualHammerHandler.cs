...
public sealed class SpiritualHammerHandler : ISpellEffectHandler
{
    public bool CanHandle(string spellId) => string.Equals(spellId, "spiritual_hammer", StringComparison.OrdinalIgnoreCase);
    ...
    public SpellCastResult Resolve(SpellCastRequest request)
    {
        ...
        if (request.Context != SpellUseContext.Combat)
            return SpellCastResult.Failure("Spiritual Hammer can only be cast in combat.");
        ...
        result.Events.Add($"{target.DisplayName} takes {actual} force damage (rolled {rolled}). HP {before}->{target.CurrentHitPoints}.");
        if (!target.IsAlive)
